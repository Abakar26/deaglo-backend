...
class DetailAdminOrganizationTest(APITestCase):
    """
    ...
    def test_detail_organization_positive(self):
        """
        Test we can get organization (non-deleted) with admin profile
        """
        ...
        self._test_response(response.data, self.organization_one)

    def test_detail_organization_positive_deleted(self):
        """
        Test we can get organization (deleted) with admin profile
        """
        self.client.force_authenticate(user=self.admin_user)
        response = self.client.get(
            reverse(
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self._test_response(response.data, self.organization_two)

    def test_detail_organization_forbidden_provider_user(self):
        """
    ...
    def test_detail_organization_forbidden_free_user(self):
        """
    ...
    def test_detail_organization_forbidden_premium_user(self):
        """
    ...
    def test_detail_organization_forbidden_org_admin_user(self):
        """
